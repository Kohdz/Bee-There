...
class TestComponent extends Component {
  ...
  handleTestCancelGoingToEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.<userUid>`]: firebase.firestore.FieldValue.delete()
      });
      toastr.success('Success');
    } catch (error) {
      // console.log(error);
  ...
  handleTestChangeAttendeePhotoInEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.<userUid>.photoURL`]: 'testing123.jpg'
      });
      toastr.success('Success');
    } catch (error) {
      // console.log(error);
  ...
  render() {
    ...
    return (
      <div>
        <h1> Test Compomnets</h1>
        <h3>The Answser is: {data}</h3>
        <Button
          name='increment'
        ...
        <br />
        <br />
        <TestPlaceInput selectAdress={this.handleSelect} />
