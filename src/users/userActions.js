  ...
  try {
    ...
    for (let i = 0; i < eventQuerySnap.docs.length; i++) {
      let eventDocRef = await firestore
        .collection('events')
        .doc(eventQuerySnap.docs[i].docs[i].data().eventId);
...
export const goingToEvent = event => async (
  ...
  try {
    ...
    await firestore.runTransaction(async transcation => {
      await transcation.get(eventDocRef);
      await transcation.update(eventDocRef, {
        [`attendees.${user.uid}`]: attendee
      });
      await transcation.set(eventAttendeeDocRef, {
        eventId: event.id,
        userUid: user.uid,
